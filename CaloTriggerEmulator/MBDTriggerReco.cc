...
MBDTriggerReco::MBDTriggerReco(const std::string &name)
  : SubsysReco(name)
...
//____________________________________________________________________________..
MBDTriggerReco::~MBDTriggerReco()
{

}

//____________________________________________________________________________..
int MBDTriggerReco::InitRun(PHCompositeNode *topNode)
{
...
//____________________________________________________________________________..
int MBDTriggerReco::process_event(PHCompositeNode *topNode)
{
	  ...
	  for (int channel = 0; channel < nchannels; channel++)
	    {
	      ...
	      output.reserve(m_nsamples);
	      for (int samp = 0; samp < m_nsamples; samp++)
		{
		  output.push_back(packet->iValue(samp, channel));
		}
	      output.push_back(waveform);
	      output.clear();
	    }
	  delete packet;
	}
  ...
  for (auto it = output.begin(); it != output.end(); it++)
    {
      std::vector<int> *prims = new std::vector<int>();
      prims->clear();
      prims->reserve(m_nsamples);
      for (int k = 0; k < m_nsamples;k++)
	{
	  prims->push_back(static_cast<int>(output[i][k]));
	}
...
void MBDTriggerReco::CreateNodeTree(PHCompositeNode *topNode)
{
  ...
  LL1Outv1 *ll1out = findNode::getClass<LL1Outv1>(detNode, "LL1_BBC");
  if (!waveforms)
    {
      ll1out = new LL1Outv1();
      PHIODataNode<PHObject> *ll1Node = new PHIODataNode<PHObject>(ll1out, "LL1_BBC", "PHObject");
      detNode->addNode(ll1Node);
    }
